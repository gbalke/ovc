  ...
  line_counter(line_counter_dev)
{
  ...
  for (int cam_id = 0; cam_id < NUM_CAMERAS; ++cam_id)
  {
    ...
    {
      cameras.insert({cam_id, std::make_unique<PiCameraV2>(i2c, vdma_devs[cam_id], cam_id, !first_camera_found)});
    ...
    if (IMX490::probe(i2c))
    {
      cameras.insert({cam_id, std::make_unique<IMX490>(i2c, vdma_devs[cam_id], cam_id, !first_camera_found)});
      first_camera_found = true;
      continue;
    }
    if (AR0521::probe(i2c))
    {
      cameras.insert({cam_id, std::make_unique<AR0521>(i2c, vdma_devs[cam_id], cam_id, !first_camera_found)});
      first_camera_found = true;
      continue;
    }
#endif
